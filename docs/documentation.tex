...
\begin{figure}[h]
\includegraphics[width=8cm]{pi4}
\centering
\caption[bottom]{Raspberry Pi 4 Model B}
\end{figure}
...
\begin{figure}[h]
    ...
    \centering
    \caption[bottom]{FTDI Adapter FT232RL from Mini-USB to TTL}
\end{figure}
...
The infrared receiver module I used is provided by ELEGOO\cite{ELEGOO}. 

\begin{figure}[h]
    \includegraphics[width=8cm]{ir_module}
    \centering
    \caption[bottom]{KY-022}
\end{figure}

IR detectors\footnote{\url{https://drive.google.com/file/d/1jfLQ6lDgM_gSl604r_zW7n1IvWrFUKN9/view}} are little microchips with a photocell that are tuned to listen to infrared light. They are almost always used for remote control detection - every TV and DVD player has one of these in the front to listen for the IR signal from the clicker. Inside the remote control is a matching IR LED, which emits IR pulses to tell the TV to turn on, off or change channels. IR light is not visible to the human eye, which means it takes a little more work to test a setup.
...
\begin{figure}[h]
    ...
    \centering
    \caption[bottom]{LCD pin assignments}
\end{figure}
...
\begin{figure}[h]
    ...
    \centering
    \caption[bottom]{Instruction Table}
\end{figure}

\subsection{IIC PCF8574T Interface}
To ease the communication to the LCD display a serial I$^2$C module\cite{i2cLcd} has been used.
...
\begin{figure}[h]
    \includegraphics[width=15cm]{schematics}
    \centering
    \caption[bottom]{Schematics}
\end{figure}
