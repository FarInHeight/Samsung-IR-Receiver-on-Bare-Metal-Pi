...
\begin{figure}[h]
    ...
    \subfigure[]{\includegraphics[width=5.52cm]{GPIO-Pinout-Diagram-2}} 
...
In an equivalent manner, you can format the micro SD card for the Pi 4 using the official software called Raspberry Pi Imager\footnote{\url{https://www.raspberrypi.com/software/}}, released by Raspberry Pi. \\
...
Once the formatting is complete\footnote{Please read the following subsection first}, you need to remove the \texttt{kernel*.img} and insert the \texttt{kernel7.img} that contains pijFORTHos.
